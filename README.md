...
这是第一阶段的完整可运行版本。程序会读取 dish_name.txt 中的菜名创意和补充说明，先生成图解01的一页菜谱主图，再按固定顺序继续生成图解02到图解06的多图解页面，最后额外输出一张竖版 2:3 的封面图，统一保存到 output 目录下。
...
17. v0.83 起，自动造菜会额外读取最近 10 条历史新菜名，自动提取其中已经出现过的主食材、口味和主烹饪方式，并把这些元素作为本轮禁用项；新菜从菜名到第二行制作描述都必须主动避开这些近历史重复元素。
18. v0.85 起，首图标题居中硬门槛改为只信豆包：本地像素偏移只保留在评分报告里作为辅助备注，不再参与首图的通过与否；图解页仍保留“AI 判断 + 本地估算”双兜底。补生图继续写回原 output 文件夹，便于直接查看新增候选图。
...
23. v0.90 起，抖音发布脚本已兼容“封面”命名：publish 目录里的封面文件不再要求必须叫 cover.*，文件名只要包含“封面”或“cover”即可被识别；并且会自动从图文上传图片里排除封面，避免误传。默认仍然不会自动点击最终“发布”按钮。
24. v0.91 起，图解01 首图链路已落地“放权模式”优化：移除文生图 prompt 的本地硬拼接注入，校验从逐字硬匹配改为“少量结构门槛 + 风险拦截”，并支持在【2人份食材】里识别和落版“配菜”分组。新增 tools/generate_page01_prompt_only.py，可只基于 dish_name.txt 生成图解01首图提示词，不走主流程、不调用图片模型。
25. v0.92 起，图解01 动态动作规则进一步放宽：不再要求数量写死或多组筷子分工，统一改为“有人手持餐具与主菜互动的瞬间”；并把“家庭切配随机性（有大有小、有长有短、厚薄不一）”升级为首图投喂和校验的固定语义，优先保证真实人类做菜与物理一致性画面。
...
11. 文本接口超时或首次请求出现 TLS/SSL 读包这类瞬时连接异常时，程序会继续使用当前同一模型在同轮内自动重试，并重建文本 client；不再依赖本地模板兜底。
12. 当前会额外生成 1 张竖版 2:3 的封面图，文件名会在菜名后追加“封面”。
13. 封面图会沿用一页菜谱 VI，并保持中轴竖排菜名与背景避让中轴的构图逻辑。
...
15. 文本请求的可重试异常判断已扩展到异常链内的 TLS/SSL EOF、httpcore NetworkError / ProtocolError 和超时；若第一次请求只是握手或读包阶段的短暂抖动，主流程会在当前这一轮内自动重试，而不是要求手动再点一次 main.py。
16. 图解02到图解06的 prompt 校验已放宽为“补硬约束优先”而不是“硬词缺失直接失败”；延续页 prompt 即使没有显式写出完整菜名，程序也会把同一道菜锚点和“不要出现程序化字段标签”负约束自动补回，再继续后续生图链路。
17. 首页与其它出现主菜的页面已新增“按菜品自适应餐具 / 手数 / 动作”的共享约束，不再固定成一双木筷夹起主菜；汤、羹、煲类可用勺子或汤匙，牛排与整块肉可用刀叉，面、粉与长条食材可用筷子、叉子或夹子，是否一只手还是多只手入镜也改为按菜品自行判断。
18. 首页步骤区已从固定“5步出锅”放宽为按菜品动态判断的 3 到 5 步，首图下半部分会按实际需要渲染【3步出锅】、【4步出锅】或【5步出锅】，减少底部文案过满的问题。
19. 图解06 的口味调整视觉不再默认固定成旋钮或滑杆，当前允许用更开放的数值可视化形式来表现辣度、酸度、浓度和清爽度，避免每套菜谱都长成同一个仪表盘模板。
20. image_generator.py 现在只保留通用素材整理、接口调用和总流程编排，图解01与封面的页面 prompt 逻辑已移动到 guide_pages 目录。
21. 图解02到图解06的本地兜底文案已改为按 dish_name.txt 输入动态生成，不再写死豆腐肉沫类固定菜文案。
22. 文本模型已临时切换为豆包新版本（doubao-seed-2-0-pro-260215），生图模型继续使用 gpt-image-2，不受影响。
23. 运行流程已调整为先产出全部创意和 prompt，再统一调用 gpt-image-2 生图。
24. 主流程已再次修正为先完成全部文案和 prompt，再统一进入所有图片的生图阶段。
25. 图解01、图解02到图解06以及封面图里只要出现任何菜品、食材、半成品、局部状态或背景食物，都已统一强制为 iPhone 主摄 1x 默认相机直出照片感。
26. 图解01 首图模板已单独强化为“像先用 iPhone 主摄拍好真实晚饭照片再排进海报里”，并保留明确互动感；但餐具类型、数量、手数和动作瞬间现在都按菜品自由变化，不再默认固定成木筷模板。
22. 图解01 首图最终 prompt 的末尾已固定追加“请优先保证主菜图片看起来像真实手机实拍，饱和度降低20%，再完成整张海报的文字和排版。”这句收尾指令。
23. 封面图模板和默认尺寸已改为竖版 2:3（1024x1536），第2张到第6张的图片 prompt 也会统一带上“关注@阿叶造新菜，家用开店都不赖！”的细长底部关注文案，不生成图标。
...
51. 封面图默认尺寸现已收口为 1024x1536。这一尺寸保持竖版 2:3，并与图解页默认尺寸一致；程序仍会在本地先校验图片尺寸配置。
...
```bash
python tools/generate_page01_prompt_only.py
```

说明：
1. 该脚本固定读取 dish_name.txt，只生成“创意菜谱 + 图解01首图文生图 prompt”两份文本。
2. 不会生成图解02到图解06，不会生成封面 prompt，不会生成发布文案。
3. 不会调用图片模型，适合手动把 prompt 投喂到 gpt-image-2 生图。
