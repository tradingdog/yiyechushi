...
def build_page01_dish_interaction_requirement() -> str:
    return (
        ...
        "只允许写一种最合适的具体餐具或上桌工具，并写出有人手持餐具与主菜互动的动作瞬间。"
...
def build_page01_plating_naturalness_requirement(bundle: dict[str, Any]) -> str:
    ...
    if has_page01_filled_main_dish(bundle) or any(keyword in combined_text for keyword in ("块", "卷", "丸", "豆腐")):
        return "主菜摆盘要像人手刚码盘：允许轻微错位、高低差、疏密变化和少量盘边汁痕；切配要有家庭处理的随机感（有大有小、有长有短、厚薄不一），不要机械等距排列。"
    return "摆盘要保留人手整理后的轻微不齐、前后错位和少量汁痕；切配要有家庭处理的随机感（有大有小、有长有短、厚薄不一），不要整齐复制。"
...
13. 主画面必须直接沿用菜谱【主画面说明】里的动态动作字段，写成一种最合适的具体餐具与互动瞬间，不要再给 gpt 列举多种餐具备选方向。
14. 如果使用筷子，不要写成“一只筷子/另一只筷子”这类不自然表达，直接写“有人手持餐具与主菜互动”即可。
15. 主标题上方的引导句必须很短，控制在 12 个汉字以内。
16. 主标题下方黄条卖点必须精简成 2 到 3 个短卖点，总长度控制在 24 个汉字以内。
17. 严禁生成“上方暖奶白标题底 + 下方矩形主图”的两段式首图，也严禁用任何横向切割把标题区和主菜照片区拆开。
17. 只有酿、包裹、夹心、内馅、卷入这类“里面本来就包着其它食材”的主菜，才允许写露芯、咬开、掰开或切开特写；像猪蹄煲、鱿鱼煲、排骨煲、整块肉这类外层主菜，不要写咬开，只写夹起、舀起、托起或盛起。
19. 器皿与摆盘必须保留真实的人手出菜痕迹，例如轻微错位、高低差、疏密变化、自然铺开或少量盘边汁痕，不要整齐复制。
20. 食材切配必须体现家庭厨房随机性：同种食材允许有大有小、有长有短、厚薄不一，不要工厂化统一切割。
...
8. 明确写出主菜要与菜谱【主画面说明】中已经指定好的唯一餐具或上桌工具产生互动，不要把多种餐具写成备选方向，也不要把动作拆成多组筷子分工的机械脚本。
...
画面里必须安排与这道菜匹配的真实餐具和主菜互动，具体餐具和动作瞬间以菜谱【主画面说明】里的动态动作字段为准；动作感必须保留，但不要再固定成所有菜都用一双木筷夹起主菜的模板。
最终 prompt 必须直接沿用菜谱【主画面说明】里的动态动作字段，不要再把筷子、勺子、刀叉、叉子、手抓写成多个备选方向；动态动作只保留一个人类互动瞬间，不要写成多组筷子分工脚本。
只有酿、包裹、夹心、内馅、卷入这类“里面本来就包着其它食材”的主菜，才允许写露芯、咬开、掰开或切开特写；像猪蹄煲、鱿鱼煲、排骨煲、整块肉这类外层主菜，不要写咬开，只写夹起、舀起、托起或盛起。
