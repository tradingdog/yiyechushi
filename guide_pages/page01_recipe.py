...
这不是“上方纯色标题底 + 下方独立主图”的两段式结构；顶部引导句、主标题、黄条卖点和收藏提示条都要直接压在同一张主菜照片的上半部，照片继续向下延伸到菜品主体区域。
...
def build_page01_chopsticks_requirement() -> str:
    return """
画面中必须保留一双木筷从画面边缘自然伸入，夹起一块主菜悬在半空，形成明显动作感和食欲点。
这个动作必须像吃饭前顺手夹起一块菜的生活化抓拍，不要拍成电商主图式的夸张英雄展示。
只保留筷子和主菜动作，不要人物脸部，不要手部特写，不要大片白雾蒸汽特效，只允许极轻微挂汁和刚出锅热感。
""".strip()
...
def build_page01_title_axis_requirement() -> str:
    return (
        ...
        "上方引导句、主标题、黄条卖点和收藏提示条要一层压一层地居中排布，"
        "每一行文字块、黄条块和收藏提示条块的中心点都要落在同一条画面正中竖线上。"
...
5. 标题下方要有一条高转化卖点副标题黄条，再下一条较细的收藏提醒条。
6. 从页面顶边到步骤条上缘的整个上半部分，必须由同一张连续主菜成品大图作为统一背景层铺开；顶部标题、黄条、收藏提示和左右卡片都直接压在这张照片上，不允许另做纯色标题底，也不允许把主菜照片做成单独带边框的照片卡片。
7. 左侧是“2人份食材”卡片，右侧是“成败关键”卡片。
8. 下半部分是“5步出锅”步骤条，5 个步骤卡横向排开，信息密度高但仍清楚。
...
13. 主画面必须出现一双筷子从画面边缘夹起一块主菜悬在半空，形成明显动作感和食欲点，但动作要像真实吃饭抓拍，不要广告式摆拍。
...
3. 左侧食材卡、右侧成败关键和“5步出锅”里的每一条文字，都必须逐项继承用户菜谱原文；食材名、数量、顺序、短句标题和步骤内容都不能自行概括、换词或改写成通用模板句。
...
9. {build_strict_centered_title_requirement('顶部引导句、主标题、黄条卖点和收藏提示条')}
...
{build_strict_centered_title_requirement('顶部引导句、主标题、黄条卖点和收藏提示条')}
{build_page01_title_axis_requirement()}
首图从页面顶边到步骤条上缘必须是一张连续主菜背景照片，顶部标题、黄条、收藏提示和左右卡片都直接压在这张照片上；不要再单独做奶白标题底，不要把主菜照片裁成带框主图。
画面里必须出现一双筷子夹起一块主菜在半空，动作感必须保留，但要像真实吃饭抓拍，不要广告式悬空摆拍。
...
右侧成败关键必须逐字使用【成败关键】里的短句；下半部分“5步出锅”必须逐条使用【5步出锅】中的标题和内容，不要改写成泛化模板步骤。
...
整体设计模板必须采用成熟爆款菜谱海报风，版式和气质沿用当前满意版本：整张上半部分是一张连续主菜背景照片，顶部双层标题区、黄条、收藏提示和左右信息卡都直接压在这张照片上，下半部分是“5步出锅”步骤条，底部是一整条收藏与关注横条。不要极简，不要杂志风，不要大面积留白，必须是信息很多但一眼就想收藏的高密度爆款图文模板。
...
主菜成品图要求：
首图从页面顶边到步骤条上缘必须共用同一张连续主菜背景照片，标题和卡片直接压在照片上；不要把照片单独裁成带框卡片，也不要把标题区另做纯色底。
